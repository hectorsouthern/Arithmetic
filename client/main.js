...
Template.admin.helpers({
    ...
    'users': function() {
        var sort = {};
        switch (Session.get('usersSortBy')) {
            case "NameAsc":
                sort = {
                    sort: {
                        username: 1
    ...
    'viewUserUsername': function() {
        return Session.get('viewUser');
    },
    'viewUserClass': function() {
        if (Session.get('viewUser') != "No User Selected") {
            return Roles.getRolesForUser(Meteor.users.find({
    ...
    'viewUserClassAvg': function() {
        if (Session.get('viewUser') != "No User Selected") {
            var role = Roles.getRolesForUser(Meteor.users.find({
                username: Session.get('viewUser')
            }).fetch()[0]._id)[0];
            var classMembers = Meteor.users.find({
                roles: role
            }).fetch()
            var totalAvg = 0;
            for (var i = 0; i < classMembers.length; i++) {
                totalAvg = totalAvg + parseInt(classMembers[i].averageScore)
            }
            var recAvg = totalAvg / classMembers.length
            return totalAvg / classMembers.length
        } else {
            return "-";
        }
    },
    ...
    'resultsSortByLabel': function() {
        if (Session.get('resultsSortByLabel') == null) {
            return "Sort By"
        } else {
            return Session.get('resultsSortByLabel');
        }
    },
    'usersSortByLabel': function() {
    ...
    'results': function() {
        ...
        var sort = {}
        var postSort = false;
        switch (Session.get('resultsSortBy')) {
            case "ScoreAsc":
                sort = {
                    sort: {
                        correct: 1
        ...
        var data = Data.find(criteria, sort).fetch()
        for (var i = data.length - 1; i >= 0; i--) {
            data[i].class = Roles.getRolesForUser(data[i].userId)[0];
            if (!Session.get('showClass1') && data[i].class == "Class 1") {
                data.splice(i, 1);
            } else if (!Session.get('showClass2') && data[i].class == "Class 2") {
                data.splice(i, 1);
            } else if (!Session.get('showClass3') && data[i].class == "Class 3") {
                data.splice(i, 1);
            }
        }
        return convertResults(data, false);
    },
    ...
    'viewUserResults': function() {
        return convertResults(Data.find({
            username: Session.get('viewUser')
        }, {
            limit: 3
        }).fetch(), true);
...
Template.admin.events({
    ...
    'submit #new-user-form': function(e, t) {
        ...
        Meteor.call('createNewUser', username, password, function(error) {
            if (error) {
    ...
    'click .btn-norounded': function(e, t) {
        Session.set('viewUser', event.target.textContent)
    },
    ...
    'click #usersSortRecAvgScoreAsc': function(e, t) {
    ...
    'click #usersSortRecAvgScoreDec': function(e, t) {
    ...
    'keyup #filterText': function(e, t) {
        if (t.find('#filterText').value == "") {
            Session.set('filterText', null);
        } else {
    ...
    'click #moveToTeacher': function() {
        Meteor.call('moveUserToRole', Session.get('viewUser'), "teacher");
    },
    'click #moveToClass1': function() {
        Meteor.call('moveUserToRole', Session.get('viewUser'), "Class 1");
    },
    'click #moveToClass2': function() {
        Meteor.call('moveUserToRole', Session.get('viewUser'), "Class 2");
    },
    'click #moveToClass3': function() {
    ...
    'click #deleteUser': function() {
        if (Session.get('viewUser') != "No User Selected") {
            BootstrapModalPrompt.prompt({
                title: "Delete User?",
                content: "Are you sure you want to delete the user: " + Session.get('viewUser')
            }, function(result) {
                if (result) {
                    Meteor.call('deleteUser', Session.get('viewUser'));
                    Session.set('viewUser', "No User Selected");
                    Session.set('progChartData', null);
                }
            });
        }
    },
    'submit #change-password-form': function(e, t) {
        e.preventDefault();
        Meteor.call('changeUserPassword', Meteor.users.find({
            username: Session.get('viewUser')
        }).fetch()[0]._id, t.find('#change-password').value);
        $('#changePasswordButton').click();
        t.find('#change-password').value = "";
    }
});
...
Template.pastresults.helpers({
    'dates': function() {
        ...
        return convertResults(data, true);
    },
    'dateconv': function(ldate) {
        var dateobj = new Date(ldate)
        return moment(dateobj).format("l LT")
...
function convertResults(data, setProgData) {
    var wrongAns = [];
    ...
    if (setProgData) {
        Session.set('progChartData', numCorrect);
    }
    for (var i = 0; i < data.length; i++) {
        dataObj.push({
            'date': data[i].date,
...
function progChartSettings() {
    return {
        ...
        series: [{
            showInLegend: false,
            type: 'spline',
            data: Session.get('progChartData'),
            color: {
